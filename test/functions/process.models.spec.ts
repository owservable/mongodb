...
jest.mock('../../src/collections.models.map');

describe('process.models.ts tests', () => {
	let mockFs: jest.Mocked<typeof import('node:fs')>;
	let mockPath: jest.Mocked<typeof import('node:path')>;
	...
	describe('edge cases', () => {
		...
		it('should map the collection and register a MongoBackend when model default export exists', () => {
			...
			const backend: any = BackendRegistry.get('testmodels');
			expect(backend).toBeInstanceOf(MongoBackend);
			expect(backend.model).toBe(mockModel);
			expect(backend.target()).toBe('testmodels');
		});

		it('should register a backend per model file', () => {
			mockListSubfoldersByName.mockReturnValue(['/test/models']);
			mockFs.readdirSync.mockReturnValue(['first.ts', 'second.js'] as any);
			mockFs.lstatSync.mockReturnValue({isDirectory: () => false} as any);

			const firstModel: any = {modelName: 'First', collection: {collectionName: 'firsts'}};
			const secondModel: any = {modelName: 'Second', collection: {collectionName: 'seconds'}};
