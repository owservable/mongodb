...
const _processMongoModels = (folder, exclude) => {
    ...
    folders.forEach((subFolder) => _processMongoModels(subFolder.fullPath, exclude));
};
const processMongoModels = (root, name = 'models', exclude) => {
    const folders = (0, folders_1.listSubfoldersByName)(root, name);
    folders.forEach((folder) => _processMongoModels(folder, exclude));
};
exports.default = processMongoModels;
